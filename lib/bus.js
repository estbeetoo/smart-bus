...
Bus.prototype.send = function(target, code, data, callback) {
  debug(`before send: ${inspect({target, code, data})}`);
  ...
  try {
    content = command.message(data);
  	...
  	console.error('Error when encoding data: %s', err.message);
    return callback(err);
  }
  ...
  debug(`send: ${message.toString('hex').toUpperCase()}`);
...
function handler(message) {
  debug(`handler msg: ${inspect(message)}`);
	try {
		var command = this.parse(message);
	} catch (err) {
		console.error('Error when parsing messages: %s', err.message);
	}
